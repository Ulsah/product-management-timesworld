				...
				<div class="modal-content">
				  ...
				  <div class="modal-body">
				  	...
				  	<form @submit.prevent="updateAppointment" @keydown="form.onKeydown($event)">
							...
							<div :class="['form-group', errors.booked_by ? 'has-error' : '']">
								<label>Booked By:</label>
								<v-select type="text" placeholder="Choose the patient" v-model="form.booked_by" label="name" :options="patients" :reduce="patients => patients.id" id="bkfld" @input="populate"></v-select>
								<span v-if="errors.booked_by" :class="['badge bg-danger']">Please choose an option.</span>
							</div>
							<div :class="['form-group', errors.name ? 'has-error' : '']">
								<label>Name:</label>
								<input type="text" class="form-control" placeholder="Enter the name" v-model="form.name" id="nmfld">
								<span v-if="errors.name" :class="['badge bg-danger']">Please enter a valid character.</span>
							</div>
							<div :class="['form-group', errors.email ? 'has-error' : '']">
								<label>Email:</label>
								<input type="email" class="form-control" placeholder="Enter the email" v-model="form.email" id="emfld">
							...
							<div :class="['form-group', errors.reason ? 'has-error' : '']">
								<label>Reason:</label>
								<input type="text" class="form-control" placeholder="Enter the reason" v-model="form.reason" id="rfld">
...
<script>
    ...
    export default {
        ...
        methods: {
        	addPrompt(){
        		document.getElementById("nmfld").disabled = false;
        		document.getElementById("emfld").disabled = false;
        		document.getElementById("bkfld").disabled = false;
        		document.getElementById("dcfld").disabled = false;
        		document.getElementById("rfld").disabled = false;
        		this.form.reset();
        		$("#addModal").modal("show");
        	},
        	
        	changePrompt(a){
        		document.getElementById("nmfld").disabled = true;
        		document.getElementById("emfld").disabled = true;
        		document.getElementById("bkfld").disabled = true;
        		document.getElementById("dcfld").disabled = true;
        		document.getElementById("rfld").disabled = true;
        		this.form.fill(a);
        		$("#addModal").modal("show");
        	},
        	...
        	getAppointments(url = "/getAppointments"){
        		axios.post(url)
        		.then(response => {
        	...
        	updateAppointment(){
        		if(this.form.id == '' || this.form.id == null){
        			...
        			this.form.post("/updateAppointment")
        			.then(response => {
        				if(response.data == 200){
        					toast.fire({
        						icon: 'success',
